...
//  FlutterAlib
//
//  Created by renan jegouzo on 23/11/2023.
//
...
class PassThrough: CommandBufferEncodable {
    ...
    func encode(
        to commandBuffer: MTLCommandBuffer, sourceTexture: MTLTexture, destinationTexture: MTLTexture
    ) {
...
class ThresholdBinary: CommandBufferEncodable {
    ...
    required init(device: MTLDevice) {
        threshold = MPSImageThresholdBinary(
            device: device,
            thresholdValue: 0.5,
            maximumValue: 1.0,
...
class MorphologyClosing: CommandBufferEncodable {
    ...
    func encode(
        ...
        let intermediateTexture = commandBuffer.device.makeTexture(
            descriptor: intermediateTextureDescriptor)!
...
class HistogramEqualization: CommandBufferEncodable {
    ...
    func encode(
        ...
        let histogramInfoBuffer = device.makeBuffer(
            length: bufferLength, options: [.storageModePrivate])!
