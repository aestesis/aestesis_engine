...
class LutUI: ModuleUI {
        ...
        if let composition = composition {
            ...
            self.io { [weak self] in
                ...
                previewSource = Bitmap(
                    parent: self, path: "assets/Luts/true.color.png", bundle: Bundle.aestesis)
            }
        }
    }
    ...
    override func process(
        ...
        guard let source = source, let input = source.output.value, let output = output.value else {
            return
        }
        guard let control = module![LutControl.asset.id],
            control.count > 0 && control.value >= 0 && Int(control.value) < assets.count
        else { return }
        guard let asset = module!.assets![Int(control.value)], let lut = assets[asset.id] else {
            return
        }
    ...
    func createPreview(lut: LutAsset) {
        bg { [weak self] in
            ...
            g.draw(
                rect: image.bounds, image: source, from: source.bounds.crop(image.bounds.ratio),
                lut: lut.texture)
            g.onDone { [weak self] _ in
                guard let self = self, self.attached else { return }
                ...
                urgent { [weak self] in
                    ...
                    let preview = Preview(
                        moduleId: id, assetId: lut.id, width: Int64(image.pixels.width),
                        height: Int64(image.pixels.height),
