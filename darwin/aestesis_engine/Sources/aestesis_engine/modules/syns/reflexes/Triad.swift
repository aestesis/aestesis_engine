...
class Triad: Reflex {
    ...
    override func draw(
        ...
        g.polygon(
            center: rect.center, count: 3,
            rotation: Signal.realTime(period: -0.1656765476, time: time).rotation, radius: mir,
            color: Color(a: 0.7 * power, l: 0))
        ...
        pb.blend = .add
        g.rosace(
            center: rect.center, count: 5,
            rotation: Signal.realTime(period: 0.1, time: time).rotation, r0: mir * power,
            r1: mar * power, paint: pb)
    }
    //////////////////////////////////////////////////////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////////////////////////////////////
    func drawArc(
        ...
        let rot =
            Signal.realTime(period: 0.0118766565776566, time: time).rotation + Signal.realTime(
                frequency: 0.001656756, time: time
            ).value * Signal.realTime(period: 0.7676587687, time: time).pow(0.1).rotation
    ...
    func drawGone(
        ...
        let da = ß.π * 2 / Double(nsec)
        let rot =
            Signal.realTime(period: 0.0167678687687, time: time).rotation + Signal.realTime(
                frequency: 0.001667565675, time: time
            ).value * Signal.realTime(period: 0.7767656556, time: time).pow(0.1).rotation
        let per = Signal.realTime(frequency: 0.1656576567, time: time).pow(0.4).value * 0.25
        for i in 0..<nsec {
            let a = da * Double(i) + rot
            let a2 = da * per
            let p: [Point] = [
                rect.center.translate(Point(angle: a, radius: mi)),
