...
class WavesKiss: WavesEffect {
    ...
    required init(parent: NodeUI, size: Size) {
        ...
        sprite = Bitmap(
            parent: self, path: "assets/Sprites/sprite-blanc.png", bundle: Bundle.aestesis)
    ...
    override func render(
        ...
        swap.blurFrom(destination: o.bounds.scale(scale), source: old, sigma: vv) { [weak self] in
            ...
            if swap.attached {
                ...
                let c = Color(
                    a: 1, r: Double(audio.eq.low * 0.08), g: Double(audio.eq.medium * 0.15),
                    b: Double(audio.eq.high * 0.15)
                ).saturated
