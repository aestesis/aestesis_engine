...
class WavesSpline : WavesEffect {
    ...
    required init(parent:NodeUI,size:Size) {
        ...
        sprite = Bitmap(parent:self,path:"Sprites/sprite-gris-16x32-no-alpha.png",bundle: Bundle(for: WavesSpline.self))
    ...
    override func render(_ time: Double, fps:Double, audio: AudioAnalyzer.Info, _ fn: @escaping ()->()) {
        if let src=swap[current&1], let dst=swap[(current+1)&1], let sprite=sprite, let output=_output {
            ...
            gp.draw(rect:spalette!.bounds, image: palette!, blend:.alpha,color:Color(a:0.1,l:1))
            
            dst.blurFrom(destination:dst.bounds.scale(1,1+0.1*20/fps),source:src,sigma:0.4) { [weak self] in
                ...
                var n=0
                sprites.append(PointSprite(position:mid,scale:spscale*Double(abs(audio.samples[s]))+spmin,color:Color.white))
                n += 1
                s += 1
                var x:Double=6
                while x < mid.x {
                    ...
                    s += 1
                    sprites.append(PointSprite(position:mid.translate(x,0),scale:ss,color:Color.white))
                    n += 1
                    sprites.append(PointSprite(position:mid.translate(-x,0),scale:ss,color:Color.white))
                    n += 1
                    x += 6
                }
                g.fill(rect:dst.bounds,blend:BlendMode.sub,color:Color(a:1,l:0.03*30/fps))
                ...
                g.onDone { [weak self] ok in
                    ...
                    self.bg { [weak self] in
                        ...
                        if output.attached {
                            ...
                            g.paletize(rect:output.bounds,source:dst,palette:self.spalette!,offset:Point(0,0),blend:BlendMode.copy,color:Color.white)
                            g.onDone { [weak self] ok in
                                guard let self=self else { return }
                                fn()
    ...
    func computerPalette() {
        ...
        for i in 0...n {
            icolors[Double(i)/Double(n)] = Color(h:ß.rnd,s:0.8,l:0.4*pow(Double(i)/Double(n),0.1)).saturated*Color(hex:"FF5020")
        }
