...
class SynCoastalWaves: Syn {
    ...
    override init(parent: NodeUI) {
        ...
        io {
            self.sprite = Bitmap(parent:self,path:"Sprites/sprite-blanc.png", bundle: Bundle(for: SynCoastalWaves.self))
            self.ghost = Bitmap(parent:self,path:"ghosts/ghost-01.png",bundle:Bundle(for:SynCoastalWaves.self))
    ...
    func drawSprite(g:Graphics,size:Size,audio:AudioAnalyzer.Info) {
        ...
        if max>0 {
            ...
            max = min(max,3)
            particles.enqueue(Particle(position: Point(x,(ß.rnd-0.5)*Double(max)*center.y+center.y), power: Double(max)*3, color:c))
        }
        if particles.count>0 {
            ...
            for p in particles {
                ...
                ps.append(PointSprite(position: position, scale: power, color: Color(a:1,rgb:p.color.lerp(to:Color.white,coef:lived)*alpha)))
            }
    ...
    func drawGhostMedium(g:Graphics,size:Size) {
        ...
        for i in 0...steps {
            ...
            var y0 = (sin(a0*1.017611)*0.5+sin(a0*0.3767133325411)*0.3+sin(a0*1.2651434515)*0.2)*rr
            var y1 = (sin(a0*1.316651561)*sin(a0*0.127615671)*0.5+sin(a0*0.1941435)*0.3+sin(a0*1.82454541)*0.2)*rr
    ...
    func drawGhostTreeble(g:Graphics,size:Size) {
        ...
        for i in 0...steps {
            ...
            var y0 = (sin(a0*1.117615)*0.5+sin(a0*0.233325411)*0.3+sin(a0*1.81434515)*0.2)*rr
            var y1 = (sin(a0*1.216651561)*sin(a0*0.317615671)*0.5+sin(a0*0.12141435)*0.3+sin(a0*1.292454541)*0.2)*rr
    ...
    func drawGhostBass(g:Graphics,size:Size) {
        ...
        for i in 0...steps {
            ...
            var y1 = (sin(a0*1.16651561)*sin(a0*0.17615671)*0.5+sin(a0*0.141435)*0.3+sin(a0*1.2454541)*0.2)*rr
    ...
    func drawLines(g:Graphics,size:Size) {
        ...
        var dr = 1.0/64
        var r = dr
        let fill:(Double,Double)->() = { r0,r1 in
            if r1>r0 && r0<=1 {
        ...
        g.draw(rect: Rect(x:0,y:0,w:size.w,h:h), image: line, color: Color.aeBlue, rotation: .anticlockwise)
        g.draw(rect: Rect(x:0,y:h,w:size.w,h:h), image: line, color: Color.aeAqua, rotation: .clockwise)
    }
